"""
Interactive voice classifier (Human vs AI). Launcher; real logic in voice.run_classifier.

Run:  python run_classifier.py          # GPU if available
      python run_classifier.py --device cpu
      python run_classifier.py path/to/audio.mp3
"""
...
# Stub torchcodec/torchaudio so DLLs are not loaded; audio is loaded with soundfile+scipy.
def _tc_dummy(name):
...
try:
    ...
    print("transformers is not installed. Install with: pip install -r requirements.txt")
    sys.exit(1)

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from voice.run_classifier import main
